...
export function BlogPostSEO({ post, baseUrl = 'https://mrguru.dev' }: BlogPostSEOProps) {
  ...
  const ogImage = (post as any).ogImage || post.coverImage || `${baseUrl}/images/mrguru-og-image.jpg`;
  ...
  const plainTextDescription = !metaDescription && post.content
    ? post.content
  ...
  const description = metaDescription || post.summary || plainTextDescription || 'Read this blog post on MrGuru.dev';
  ...
  const seoKeywords = (post as any).keywords && Array.isArray((post as any).keywords)
  ...
  const safeTitle = `${title} | MrGuru.dev Blog`;
  ...
  useEffect(() => {
    ...
    updateMetaTag('keywords', keywords ? `${keywords}, MrGuru.dev, Anthony Feaster, blog, web development` : 'MrGuru.dev, Anthony Feaster, blog, web development');
    ...
    // Add article tags
    const tagsToUse = seoKeywords.length > 0 ? seoKeywords : (post.tags && Array.isArray(post.tags) ? post.tags : []);
    ...
    updateJsonLdScript({
      ...
      'keywords': keywords || 'MrGuru.dev, Anthony Feaster, blog, web development',
      'author': {
        '@type': 'Person',
    ...
    return () => {
      document.title = 'MrGuru.dev | Portfolio & Blog';
    };
  }, [safeTitle, description, keywords, postUrl, ogTitle, ogImage, publishDate, modifiedDate, seoKeywords, baseUrl, canonicalUrl, twitterCard]);
