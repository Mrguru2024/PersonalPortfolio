        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3].map((i) => (
              <Card
                key={i}
                className="border border-gray-200 dark:border-gray-700 h-[400px] animate-pulse"
              >
          ...
          <div className="text-center py-10">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              Check back soon for new content! I'm working on exciting articles
              to share with you.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {blogPosts.map((post: BlogPost) => (
              <Card
                key={post.id}
                className="overflow-hidden flex flex-col h-full border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow duration-300"
              >
                {post.coverImage ? (
                ...
                <CardFooter className="flex flex-col items-start space-y-3 pt-0">
                  ...
                  <Link href={`/blog/${post.slug}`}>
                    <Button
                      variant="link"
                      className="h-auto p-0 font-normal text-primary hover:text-primary/80"
                    >
                      Read more →
                    </Button>
                  </Link>
        ...
        <div className="mt-12 text-center">
          <Link href="/blog">
            <Button
              variant="outline"
              size="lg"
              className="rounded-full text-sm sm:text-base min-h-[44px] w-full sm:w-auto"
            >
              View All Posts
            </Button>
          </Link>
