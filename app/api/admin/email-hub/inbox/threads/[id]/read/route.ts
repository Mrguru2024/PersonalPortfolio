...
import { setEmailHubProviderThreadReadState } from "@server/services/emailHub/mailbox/emailHubMailboxActions";
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const internalThreadId = Number((await ctx.params).id);
  ...
  if (!Number.isFinite(internalThreadId) || mailboxId == null || !Number.isFinite(Number(mailboxId))) {
  ...
  const r = await setEmailHubProviderThreadReadState({
