...
import { AdminPlatformTips } from "@/components/admin/AdminPlatformTips";
...
export default function EditPlaybookPage() {
  ...
  useEffect(() => {
    ...
    setServiceType(playbook.serviceType ?? "");
    ...
    setQualificationRules(playbook.qualificationRules ?? "");
    setRedFlags(playbook.redFlags ?? "");
    setProposalRequirements(playbook.proposalRequirements ?? "");
    setFollowUpGuidance(playbook.followUpGuidance ?? "");
    setActive(playbook.active !== false);
  }, [playbook]);
  ...
  const aiGenerateMutation = useMutation({
    ...
    onSuccess: (data: {
      ...
      toast({ title: "AI content generated", description: "Review and edit, then save." });
    },
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("PATCH", `/api/admin/crm/playbooks/${id}`, {
        title: title.trim(),
        slug: slug.trim(),
        category: category || null,
        serviceType: serviceType || null,
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/crm/playbooks"] });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/crm/playbooks", id] });
      toast({ title: "Playbook updated" });
      router.push(`/admin/crm/playbooks/${id}`);
    },
      ...
      <div className="container mx-auto min-w-0 px-3 fold:px-4 sm:px-6 py-8 max-w-2xl">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <Button variant="ghost" size="sm" asChild>
            <Link href={`/admin/crm/playbooks/${id}`}><ArrowLeft className="h-4 w-4 mr-2" />Back to playbook</Link>
          </Button>
        </div>

        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><BookOpen className="h-5 w-5" /> Edit playbook</CardTitle>
            <CardDescription>Update this playbook. Use &quot;Generate with AI&quot; to refill content from title and type.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">Refills description, checklist, rules, guidance, and platform usage tips.</span>
            </div>
              ...
              <ul className="space-y-2">
                {checklistItems.map((item, i) => (
                  <li key={i} className="flex gap-2">
                    <Input value={item} onChange={(e) => setChecklistItem(i, e.target.value)} placeholder="e.g. Confirm budget" />
            ...
            <div>
              ...
              <Textarea value={qualificationRules} onChange={(e) => setQualificationRules(e.target.value)} rows={2} />
            </div>
            <div>
            ...
            <div>
              ...
              <Textarea value={proposalRequirements} onChange={(e) => setProposalRequirements(e.target.value)} rows={2} />
            </div>
            <div>
              <Label>Follow-up guidance</Label>
              <Textarea value={followUpGuidance} onChange={(e) => setFollowUpGuidance(e.target.value)} rows={2} />
            </div>
            <div className="flex items-center gap-2">
              <Switch id="active" checked={active} onCheckedChange={setActive} />
              <Label htmlFor="active">Active</Label>
            </div>
            <Button onClick={() => updateMutation.mutate()} disabled={!title.trim() || updateMutation.isPending}>
