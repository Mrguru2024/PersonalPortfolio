...
const FloatingNavigation: React.FC = () => {
      ...
      if (validSections.length > 0) {
        ...
        const current = validSections.reduce((prev, current) => {
          ...
          return prevBounds.top < 0 && currentBounds.top < 0
            ? prevBounds.top > currentBounds.top
              ? prev
              : current
            : prevBounds.top < currentBounds.top && prevBounds.top >= 0
              ? prev
              : current;
        });
  ...
  return (
    <>
      ...
      <motion.div
        className="fixed bottom-14 right-4 z-40 bg-primary text-white p-3 rounded-full shadow-lg md:hidden cursor-pointer min-h-[44px] min-w-[44px] flex items-center justify-center touch-target"
      ...
      <motion.div
        className="fixed bottom-14 left-4 z-50 bg-primary text-white p-3 rounded-full shadow-lg md:hidden cursor-pointer min-h-[44px] min-w-[44px] flex items-center justify-center touch-target"
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        onClick={scrollToTop}
        whileTap={{ scale: 0.9 }}
