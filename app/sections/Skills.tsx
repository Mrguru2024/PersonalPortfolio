...
import { frontendSkills, backendSkills, devopsSkills } from "@/lib/data";

function getStaticSkills(): Skill[] {
  const toSkill = (
  ...
  let id = 0;
  return [
    ...frontendSkills.map((s) => toSkill(s, "frontend", ++id)),
    ...backendSkills.map((s) => toSkill(s, "backend", ++id)),
    ...devopsSkills.map((s) => toSkill(s, "devops", ++id)),
  ];
}
...
export default function Skills() {
  const { data: skills, isLoading } = useQuery<Skill[]>({
    ...
    queryFn: async () => {
      try {
        ...
        if (!res.ok) return getStaticSkills();
  ...
  const categorizedSkills = {
    ...
    other:
      skills?.filter(
        (skill) =>
          !["frontend", "backend", "database", "devops", "design"].includes(
            skill.category
          )
      ) || [],
  };
  ...
  return (
    <section
      id="skills"
      className="py-20 bg-gradient-to-b from-background/90 to-background/95"
    >
      <div className="container px-4 mx-auto">
        ...
        >
          ...
          <motion.p
            ...
            variants={itemVariants}
          >
            I've spent years developing a diverse set of skills that allow me to
            handle every aspect of web development, from frontend design to
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {categories.map((category) => {
                const categorySkills =
                  categorizedSkills[
                    category.key as keyof typeof categorizedSkills
                  ];
