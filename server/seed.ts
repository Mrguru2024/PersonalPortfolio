...
async function seedProjects() {
  ...
  try {
    ...
    if (error?.code === "42P01") {
      console.log(
        "Projects table does not exist yet. Please run: npm run db:push"
      );
      return;
    }
    throw error;
...
async function seedSkills() {
  ...
  console.log(
    `Seeded ${
      frontendSkills.length + backendSkills.length + devopsSkills.length
    } skills`
  );
}
...
async function seedBlogPosts() {
  ...
  if (existingUser.length === 0) {
    ...
    if (!seedAdminEmail || !seedAdminPassword) {
      console.log(
        "⚠️  Skipping admin user creation - SEED_ADMIN_EMAIL and SEED_ADMIN_PASSWORD not set"
      );
      console.log(
        "   To create a seed admin user, set these environment variables in .env.local"
      );
    } else {
  ...
  // Helper: estimate reading time in minutes from HTML content
  const readingTimeFromContent = (html: string) =>
    Math.max(
      1,
      Math.round(
        html
  ...
  // Seed blog posts: full HTML, SEO-ready, with internal + external backlinks
...
<p>If you’re considering a similar move, take it step by step and test at each stage. Need a hand with your own site? Check out <a href="/#services">my web development services</a> or <a href="/#contact">get in touch</a> for a quote.</p>`,
      ...
      isPublished: true,
      metaTitle:
        "Migrating from Vite to Next.js 15: Complete Guide | MrGuru.dev",
      metaDescription:
...
<p>Need help designing or refactoring APIs? See <a href="/#services">my services</a> or <a href="/#contact">contact me</a> and we can go through your project.</p>`,
  ...
  for (const post of blogPostsToSeed) {
    ...
    if (inserted && post.slug === "migrating-vite-to-nextjs-complete-guide") {
      firstPostId = inserted.id;
    }
  }
    ...
    await db.insert(blogComments).values([
      {
        ...
        content:
          "Great guide! I'm planning a similar migration. How long did the entire process take?",
      ...
      },
      {
        ...
        content:
          "This is exactly what I needed! The section on handling SSR was particularly helpful.",
