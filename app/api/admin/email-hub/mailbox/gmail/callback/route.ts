...
import { connectGmailMailboxFromCode } from "@server/services/emailHub/mailbox/emailHubMailboxConnect";
import {
...
export async function GET(req: NextRequest) {
  ...
  const saved = await connectGmailMailboxFromCode(user.id, code, redirectUri);
  if (!saved.ok) {
    return redirectInbox(baseUrl, `?mailbox_error=${encodeURIComponent(saved.error)}`);
  }
  return redirectInbox(baseUrl, "?mailbox_connected=gmail");
}
