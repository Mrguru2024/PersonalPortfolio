  ...
  const { data: projects, isLoading } = useQuery<Project[]>({
    ...
    queryFn: async () => {
      try {
        ...
        if (!res.ok) return staticProjects as Project[];
        const json = await res.json();
        return Array.isArray(json) ? json : (staticProjects as Project[]);
      } catch {
        return staticProjects as Project[];
      }
    },
    staleTime: 60_000,
  });
      ...
      <div className="container px-4 mx-auto">
        ...
        >
          <AnimatedButton
            ...
            withHoverEffect
            onClick={() =>
              window.open("https://github.com/Mrguru2024", "_blank")
            }
          >
...
function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <Card className="h-full overflow-hidden border border-border/50 bg-card/50 backdrop-blur-sm hover:border-primary/50 transition-all duration-300 transform hover:-translate-y-1 group/card">
      <Link href={`/projects/${project.id}`} className="block">
        ...
        <CardHeader className="p-4 pb-2">
          <div className="flex justify-between items-start">
            <CardTitle className="text-xl group-hover/card:text-primary transition-colors duration-300">
              {project.title}
            </CardTitle>
            <span
              className={cn(
