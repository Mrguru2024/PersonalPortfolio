...
const CustomCursor = ({ currentSection }: CustomCursorProps) => {
  ...
  // Hide help after some time - ALWAYS call this hook (even if component will return null)
  useEffect(() => {
    if (showHelp && !isTouchDevice) {
  ...
  const getColorAtPosition = useCallback((x: number, y: number) => {
    ...
    try {
      ...
      const isPrimaryColor = (r: number, g: number, b: number): boolean => {
        ...
        return (
          (b > 200 && r < 100 && g < 150) || (r > 200 && b > 200 && g < 150)
        ); // Purple/blue range
      };
  ...
  // Mouse event handlers - ALWAYS call this hook (even if component will return null)
  useEffect(() => {
    // Don't set up event listeners on touch devices or mobile
    if (
      isTouchDevice ||
  ...
  }, [handleHover, getColorAtPosition, isTouchDevice]);

  // Hide cursor on touch devices or mobile - AFTER all hooks are called
  if (
    isTouchDevice ||
    (typeof window !== "undefined" && window.innerWidth < 768)
  ) {
