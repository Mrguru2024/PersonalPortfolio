...
export const StructuredData: React.FC<StructuredDataProps> = ({ schema }) => {
  ...
  useEffect(() => {
    ...
    // Create a unique ID for this structured data based on schema type and key data
    const schemaId = `structured-data-${schema.type}-${schema.type === 'Person' ? schema.data.name : schema.type === 'WebSite' ? 'website' : schema.type === 'BlogPosting' ? schema.data.headline : 'default'}`;
    
    // Remove existing script with this ID if it exists
    const existingScript = document.querySelector(`script[type="application/ld+json"][data-seo-id="${schemaId}"]`);
    if (existingScript) {
    ...
    script.setAttribute('data-seo-id', schemaId);
    script.textContent = JSON.stringify(schemaData);
    document.head.appendChild(script);
    
    // Cleanup function to remove script when component unmounts
    return () => {
      const scriptToRemove = document.querySelector(`script[type="application/ld+json"][data-seo-id="${schemaId}"]`);
      if (scriptToRemove) {
